...
def build_vae_var_from_config(device, model_cfg: OmegaConf.DictConfig):

    vqvae = model_cfg.vqvae
    var = model_cfg.var

    return build_vae_var(
        device,
        patch_nums=model_cfg.patch_nums,
        V=vqvae.V, Cvae=vqvae.Cvae, ch=vqvae.ch, share_quant_resi=vqvae.share_quant_resi,
        num_classes=var.num_classes, depth=var.depth, shared_aln=var.shared_aln, attn_l2_norm=var.attn_l2_norm,
    )
